(function () {
  // Base URL the loader was served from — the DIRECTORY of its own <script src>,
  // not merely the origin. Every backend call is resolved against this, so the
  // widget follows whatever path it is published under and this file contains no
  // absolute host of its own:
  //
  //   https://host/widget.js                     → https://host/api/…
  //   https://portal/campusagents/widget.js      → https://portal/campusagents/api/…
  //
  // The second case is what makes a reverse proxy on the embedding site work: the
  // portal maps one prefix to this deployment, and the loader keeps every request
  // inside that prefix instead of reaching for the portal's root (where the
  // portal's own application answers, not ours). Anchoring on `.origin` used to
  // discard the path and did exactly that.
  //
  // The prefix is deliberately NOT known here: the embedding site picks it and can
  // change it without a coordinated release on our side.
  //
  // Captured at top level, while document.currentScript is still valid.
  let scriptBase = '';
  try {
    const cs = document.currentScript;
    // new URL('.', src) = the script's directory, with a trailing slash.
    if (cs && cs.src) scriptBase = new URL('.', cs.src).href;
  } catch (e) { /* ignore */ }
  if (!scriptBase) scriptBase = window.location.origin + '/';
  ...
  // 1. No web font is loaded. This used to inject a stylesheet <link> to
  //    fonts.googleapis.com, which is an absolute third-party host: on an
  //    embedding page it is blocked by any strict style-src/font-src CSP, and it
  //    leaks every visitor's IP to Google (not acceptable on a JLU page). The
  //    widget's font-family below therefore starts with 'Inter' — used when the
  //    host page already provides it — and otherwise falls through to the
  //    platform UI font. Icons are inline Lucide SVGs (generated into ICONS
  //    above), so no icon font is needed either. Net result: this loader issues
  //    ZERO off-domain requests.
  ...
  async function initWidget(containerEl) {
    ...
    // API base, resolved RELATIVE to the loader's own URL (see scriptBase above),
    // so it lands under the same path prefix widget.js was served from.
    // data-api overrides it, and is only needed when the API is NOT reachable
    // under that prefix — then it must be an absolute URL to the CampusAgents
    // host, which additionally requires the embedding origin in ALLOWED_ORIGINS
    // (see docs/DEPLOYMENT.md).
    const apiBase = (containerEl.getAttribute('data-api') || new URL('api', scriptBase).href).replace(/\/+$/, '');
